...
export default function WhatsAppPreview({ senderName, showIdentity, receiverName }) {
  ...
  const doodle = "url(\"data:image/svg+xml," + encodeURIComponent(
    ...
    "<circle cx='108' cy='23' r='1.4' fill='#CBBBA4'/><circle cx='116' cy='23' r='1.4' fill='#CBBBA4'/>" +
  ...
  const who = (showIdentity && senderName && senderName.trim()) ? senderName.trim() : "Someone";
  const greetName = (receiverName && receiverName.trim()) ? receiverName.trim() : "receiver";

  return (
    <div style={{ borderRadius: 16, overflow: "hidden", border: "1px solid rgba(5,5,5,0.08)", background: "#E4DDD3", fontFamily: WA_SANS }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "9px 12px",
        background: "#F6F6F6", borderBottom: "1px solid rgba(0,0,0,0.08)", color: "#0A0A0A" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 2, flex: "none", fontSize: 17 }}>
      ...
      <div style={{ padding: "16px 12px 18px", backgroundColor: "#E4DDD3", backgroundImage: doodle }}>
        <div style={{ position: "relative", maxWidth: "86%", background: "#FFFFFF", borderRadius: "0 8px 8px 8px",
          padding: "7px 9px 6px 11px", boxShadow: "0 1px 0.5px rgba(0,0,0,0.13)", fontSize: 13.5, lineHeight: 1.4, color: "#111B21" }}>
          <span style={{ position: "absolute", top: 0, left: -7, width: 0, height: 0, borderStyle: "solid",
            borderWidth: "0 8px 8px 0", borderColor: "transparent #FFFFFF transparent transparent" }} />
          <span style={{ whiteSpace: "pre-wrap" }}>
            Hi {greetName}, {who} has created a surprise just for you. Solve the puzzle to reveal what's waiting inside. 🧩{" "}
