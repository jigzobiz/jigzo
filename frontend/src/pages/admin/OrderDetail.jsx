...
import { PageHeader, Card, StatGrid, StatTile, Badge, Loading, ErrorNote, useAdminData, adminGet, formatBHD, formatMoney, formatDate, T } from './adminShared';
...
export default function OrderDetail() {
  ...
  return (
    <>
      ...
      <PageHeader title={`Order ${order.orderId}`} explain="Read-only detail: payment summary and every recipient puzzle. The displayed (localised) price and the exact BHD charged are shown separately." />

      <StatGrid min={180}>
        <StatTile label="Payment" value={order.paymentStatus} tone={order.completed ? 'good' : 'neutral'} />
        <StatTile label="Charged (BHD)" value={payment.amountBHD ? formatBHD(payment.amountBHD) : '—'} tone={order.completed ? 'good' : undefined} />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 16, marginTop: 20 }}>
        <Card title="Payment summary">
          <Row label="Displayed price" value={payment.displayAmount ? formatMoney(payment.displayCurrency, payment.displayAmount) : '—'} />
          <Row label="Actual amount charged" value={payment.amountBHD ? formatBHD(payment.amountBHD) : '—'} />
      ...
      <Card title={`Puzzles in this order (${puzzles.length})`} style={{ marginTop: 16 }}>
        {puzzles.length === 0 ? <div style={{ color: T.ink50, fontSize: 13.5, padding: 12 }}>No puzzle recipients found.</div>
          : puzzles.map((p) => (
            <div key={p.index} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, flexWrap: 'wrap', borderBottom: `1px solid ${T.ink08}`, padding: '12px 0' }}>
              <div><span style={{ fontWeight: 700 }}>Puzzle {p.index + 1} · {p.recipientName}</span><span style={{ color: T.ink50, fontSize: 12.5, marginLeft: 8 }}>via {p.deliveryMethod}</span></div>
              <div style={{ display: 'flex', gap: 8, alignItems: 'center', fontSize: 12.5, color: T.ink66 }}>
                <Badge tone={stateTone(p.state)}>{p.state}</Badge>
                <span style={{ color: T.ink50 }}>delivery: {p.deliveryTracking}</span>
              </div>
            </div>
          ))}
      </Card>
