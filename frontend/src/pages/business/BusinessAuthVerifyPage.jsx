...
export default function BusinessAuthVerifyPage() {
  ...
  useEffect(() => { const params = new URLSearchParams(window.location.hash.slice(1)); const token = params.get('token') || ''; window.history.replaceState({}, '', '/business/auth/verify'); if (!token) { setFailed(true); return; } businessApi.verifyMagicLink(token).then(() => navigate('/business/campaigns/new', { replace: true })).catch(() => setFailed(true)); }, [navigate]);
