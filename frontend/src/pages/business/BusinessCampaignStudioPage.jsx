...
function RecipientsArea({ copy, isArabic }) {
  const { state, dispatch, recipientActions } = useCampaignStudio(); const fileRef = useRef(null); const [drafts,setDrafts]=useState([]); const [edits,setEdits]=useState({}); const [rowErrors,setRowErrors]=useState({}); const [error,setError]=useState('');
  ...
  const change=(id,field,value,isDraft)=>{if(isDraft)setDrafts(rows=>rows.map(row=>row.localId===id?{...row,[field]:value}:row));else setEdits(rows=>({...rows,[id]:{...rows[id],[field]:value}}));setRowErrors(rows=>({...rows,[id]:{...rows[id],[field]:undefined}}));};
  const saveRow=async(row,isDraft)=>{const id=isDraft?row.localId:row.recipientId;const fields=validation(row);if(Object.keys(fields).length){setRowErrors(value=>({...value,[id]:fields}));return;}try{if(isDraft){await recipientActions.create(row);setDrafts(value=>value.filter(item=>item.localId!==id));}else await recipientActions.update(id,row);setRowErrors(value=>({...value,[id]:{}}));}catch(e){const message=e.response?.data?.code?.includes('DUPLICATE')||e.response?.status===409?(isArabic?'مستلم مكرر':'Duplicate recipient'):(e.response?.data?.error||copy.recipients.failed);setRowErrors(value=>({...value,[id]:{contact:message}}));}};
  ...
  return <><AreaIntro copy={copy} index={3} /><div className="jzs-recipient-actions"><button className="jzs-action jzs-action--ghost" type="button" onClick={() => fileRef.current?.click()}>{copy.recipients.upload}</button><input ref={fileRef} hidden type="file" accept=".csv,text/csv" onChange={upload} /><button className="jzs-text-action" type="button" onClick={download}>{copy.recipients.template}</button><span>{rows.length} {isArabic?'مستلم':'recipients'}{attention?` · ${attention} ${isArabic?'يحتاج إلى انتباه':'need attention'}`:''} · 2,000 max</span></div>
    ...
    <div className="jzs-recipient-grid" role="table"><div className="jzs-recipient-grid__head" role="row"><span>{copy.recipients.name}</span><span>{copy.recipients.contactMethod}</span><span>{isArabic?'بيانات التواصل':'Contact'}</span><span>+1</span><span /></div>{rows.map(row=>{const isDraft=Boolean(row.localId),id=row.localId||row.recipientId,fields=rowErrors[id]||{};return <div className={`jzs-recipient-row${Object.values(fields).some(Boolean)?' has-error':''}${state.studio.selectedRecipientId===row.recipientId?' is-selected':''}`} role="row" key={id} onFocus={()=>row.recipientId&&dispatch({type:'SELECT_RECIPIENT',id:row.recipientId})}><label><span>{fields.name}</span><input aria-label={copy.recipients.name} value={row.name} onChange={e=>change(id,'name',e.target.value,isDraft)}/></label><label><span>{fields.contactMethod}</span><select aria-label={copy.recipients.contactMethod} value={row.contactMethod} onChange={e=>change(id,'contactMethod',e.target.value,isDraft)}><option value="email">Email</option><option value="whatsapp">WhatsApp</option></select></label><label><span>{fields.contact}</span><input className="jzs-ltr" aria-label={isArabic?'بيانات التواصل':'Contact'} type={row.contactMethod==='email'?'email':'tel'} value={row.contact} placeholder={row.contactMethod==='whatsapp'?'+973XXXXXXXX':'name@example.com'} onChange={e=>change(id,'contact',e.target.value,isDraft)}/></label><label><span /><select className={`jzs-plus-one is-${row.plusOneOverride}`} aria-label={copy.recipients.plusOne} value={row.plusOneOverride} onChange={e=>change(id,'plusOneOverride',e.target.value,isDraft)}><option value="inherit">{copy.recipients.inherit}</option><option value="allowed">ON</option><option value="not_allowed">OFF</option></select></label><div className="jzs-recipient-row__actions"><button type="button" onClick={()=>saveRow(row,isDraft)}>{copy.recipients.save}</button><button type="button" onClick={()=>isDraft?setDrafts(value=>value.filter(item=>item.localId!==id)):recipientActions.remove(id)}>×</button></div></div>})}</div>
