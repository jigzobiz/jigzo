...
function RecipientsArea({ copy, isArabic }) {
  ...
  const addSelectedContacts = async selected => {
    ...
    const accepted = []; let duplicateCount = 0; let overLimit = 0;
    for (const contact of selected) {
      const parsed = internationalPhone(contact.phone);
      const identity = parsed.e164 || parsed.raw;
      if (identity && existing.has(identity)) { duplicateCount++; continue; }
      if (rows.length + accepted.length >= 2000) { overLimit++; continue; }
      if (identity) existing.add(identity);
      accepted.push({ localId: crypto.randomUUID(), name: contact.name || '', contactMethod: 'whatsapp',
    ...
    if (duplicateCount) notices.push(isArabic ? `تم تخطي ${duplicateCount} من الأرقام المكررة.` : `${duplicateCount} duplicate number(s) were skipped.`);
    if (pending.length) notices.push(isArabic ? 'راجع الأرقام التي تحتاج إلى تعديل أدناه.' : 'Review the editable numbers that need attention below.');
    if (saved && !notices.length) notices.push(isArabic ? `تمت إضافة ${saved} مستلم.` : `${saved} recipient(s) added.`);
  ...
  return <div className="jzs-recipients-workspace">
    <div className="jzs-recipient-toolbar">
      ...
      <div className="jzs-recipient-toolbar__actions">
        ...
        <ContactPickerButton onSelect={addSelectedContacts} isArabic={isArabic} />
